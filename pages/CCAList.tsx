...
const CCAList: React.FC = () => {
  ...
  const [heroSettings, setHeroSettings] = useState({
    image: 'https://images.unsplash.com/photo-1566417713940-fe7c737a9ef2?q=80&w=2000',
    title: 'CCA 소개',
    subtitle: '필리핀 전역 최고의 JTV 파트너사와 함께하는 프리미엄 CCA를 만나보세요. 엄격한 기준으로 선발된 최고의 인재들이 여러분을 기다립니다.'
  });
  ...
  useEffect(() => {
    const fetchCCAs = async () => {
      ...
      try {
        ...
        if (settings) {
          setHeroSettings({
            image: settings.ccas_hero_image || 'https://images.unsplash.com/photo-1566417713940-fe7c737a9ef2?q=80&w=2000',
            title: settings.ccas_hero_title || 'CCA 소개',
            subtitle: settings.ccas_hero_subtitle || '필리핀 전역 최고의 JTV 파트너사와 함께하는 프리미엄 CCA를 만나보세요. 엄격한 기준으로 선발된 최고의 인재들이 여러분을 기다립니다.'
          });
        }
      } catch (error) {
    ...
    <div className="min-h-screen bg-background-light dark:bg-background-dark">
      ...
      <section className="relative w-full h-[300px] md:h-[500px] bg-background-dark overflow-hidden flex flex-col justify-end">
        ...
        <div className="relative max-w-7xl mx-auto w-full px-4 md:px-8 pb-12 md:pb-20 space-y-4">
          ...
          <h2 className="text-4xl md:text-7xl font-extrabold text-white tracking-tighter">{heroSettings.title}</h2>
